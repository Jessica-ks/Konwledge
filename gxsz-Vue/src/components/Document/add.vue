        ...
        <div class="box-body">
          ...
          <div class="row">
            <div class="col-md-6  col-sm-6  col-xs-12">
              <div class="form-group" :class="{'has-error':targetTips}">
              <label>选择群组</label>
                <el-select v-model="group" multiple placeholder="请选择群组...">
                  <el-option  v-for="item in groupArr" :label="item.label" :value="item.value" :key="item.value">
                  </el-option>
                </el-select>
              </div>
            </div>
            <div class="col-md-6  col-sm-6  col-xs-12">
              <div class="form-group" :class="{'has-error':targetTips}">
                <!-- <label>已选群组</label>
                <el-select v-model="per_label" @change="changePerson" filterable multiple remote
                placeholder="请在左侧选择群组..."></el-select>
                <span class="help-block" style="text-align:center" v-show="targetTips">{{targetTips}}</span> -->
                <!-- <label>  </label>
                <el-button size="small" type="info" @click='go("Users/AdminUsers/myGroup/1")' style="margin-top:25px;height:36px;font-weight:bold;font-size:14px">新建群组</el-button> -->
              </div>
            </div>
          </div>
          <div class="row">
  ...
  export default {
    ...
    methods: {
      ...
      formSubmit() {
        ...
        //保证发布对象不能为空
        _this.targetTips = _this.nowSch.length == 0 && _this.nowDep.length == 0 && _this.nowPer.length == 0 && _this.group.length == 0 ? '请至少选择一个发布对象!' : null;
        if (checkResult && !_this.targetTips) {
          _this.Loading = true;
          //发送提交表单请求
          var config = {
            method: 'post',
            url: '/index.php?g=document&m=Admindocument&a=addPost',
            params: {

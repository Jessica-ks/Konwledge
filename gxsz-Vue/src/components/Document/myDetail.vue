          ...
          <div class="row">
            <div class="col-md-offset-1 col-md-10 col-sm-offset-1 col-sm-8 col-xs-12" style="text-align:center">
              ...
              <div class="desc-wrapper" style="margin-bottom:16px;font-size:14px;">
                <span>发表人:{{DataObj.user_nicename}}</span> <span style="margin-left:14px">发表时间:{{DataObj.create_time}}</span>
              </div>
              ...
              <div class="footer" style="margin-top:16px" v-if='DataObj.roleStatus==0'>
              ...
              <el-select  :placeholder="'已读人数'+DataObj.read">
                <el-option v-for="item in DataObj.readman" :label="item.label" :value="item.value" :key="item.value"></el-option>
              </el-select>
              <el-select  :placeholder="'未读人数'+DataObj.unread">
                <el-option v-for="item in DataObj.unreadman" :label="item.label" :value="item.value" :key="item.value"></el-option>
              </el-select>
              <el-select  :placeholder="'转发量'+DataObj.handle">
                <el-option v-for="item in DataObj.handleman" :label="item.label" :value="item.value" :key="item.value"></el-option>
              </el-select>
              </div>
            </div>
          </div>
 ...
 export default{
  ...
  methods:{
   ...
   created(){
     ...
     var initConfig = {
       "method": "get",
       "url": "/index.php?g=document&m=Admindocument&a=docDetail",
       "params": {

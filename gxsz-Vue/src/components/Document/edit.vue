            ...
            <div class="col-md-4 col-md-offset-2 col-sm-4 col-sm-offset-2 col-xs-12">
              <div class="form-group">
                ...
                <div class="input-group radio-height">
                  ...
                  off-text="否" :disabled="DataObj.send_status==1"></el-switch>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="col-md-6  col-sm-6  col-xs-12">
              <div class="form-group" :class="{'has-error':targetTips}">
                ...
                <el-cascader :options="perTree" @active-item-change="handleItemChange" filterable @change="selectPerson"
                             style="width:100%;border-radius:0" placeholder="请选择人员..." :disabled="DataObj.send_status==1"></el-cascader>
              </div>
            </div>
            <div class="col-md-6  col-sm-6  col-xs-12">
              <div class="form-group" :class="{'has-error':targetTips}">
                ...
                <el-select v-model="per_label" @change="changePerson" filterable multiple remote
                           placeholder="请在左侧选择人员..." :disabled="DataObj.send_status==1"></el-select>
          ...
          <div class="row">
            <div class="col-md-6  col-sm-6  col-xs-12">
              <el-upload class="upload-demo" :on-remove="removeThis" ref="imgUpload" :on-success="removeBefore"
                         action="/gxsz_office/index.php?g=document&m=Admindocument&a=uploadfile" :file-list="fileArr" :disabled="DataObj.send_status==1">
                <el-button size="small" type="primary" :disabled="DataObj.send_status==1">上传附件</el-button>
              </el-upload>
  ...
  export default {
    ...
    methods: {
      ...
      removeThis(file, list) {
        var _this = this;
        if(_this.DataObj.send_status!=1){
          var config = {
            method: 'get',
      ...
      formSubmit() {
        ...
        if (checkResult && !_this.targetTips) {
          ...
          var config = {
            method: 'post',
            url: '/index.php?g=document&m=Admindocument&a=editPost',
            params: {
    ...
    created() {
      ...
      _this.myHttp(config,
        function () {
          ...
          var labelArr=[];
          //初始化数据
          _this.DataObj.user_info.forEach(function(person){
            //这里的人员一定是存在的,所以不用进行判断
              labelArr.push(person.label);
              _this.nowPer.push(person.value);
              _this.tempPerArr.push(person);
          });
         
          _this.nowTitle = _this.DataObj.title;
          _this.nowContent = _this.DataObj.content;
          _this.isSave = _this.DataObj.send_status==1;
          _this.per_label=labelArr;
